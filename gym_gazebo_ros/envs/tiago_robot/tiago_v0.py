...
class TiagoEnv(gym.Env):
    ...
    def get_ee_state(self):
        ...
        if self.__robot_name == 'steel':
            ...
            try:
                end_state = self.__get_link_pose_srv.call('tiago_steel::gripper_link', "base_footprint").link_state
            except (rospy.ServiceException) as exc:
                print("/gazebo/get_link_state service call failed:" + str(exc))
        else:
            print('End effector is a 5 finger hand....')
            try:
                end_state = self.__get_link_pose_srv.call('tiago_titanium::hand_index_link', "base_footprint").link_state
            except (rospy.ServiceException) as exc:
                print("/gazebo/get_link_state service call failed:" + str(exc))
