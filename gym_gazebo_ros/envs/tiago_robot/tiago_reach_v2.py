...
class TiagoReachV2(TiagoEnv):
    ...
    def __init__(self):
        ...
        for i, joint in enumerate(self.robot.joints):
            ...
            if joint.joint_type != 'fixed' and (
                ...
                self.__joint_limits_upper.append(
                    joint.limit.upper) if joint.limit.upper is not None else self.__joint_limits_upper.append(
                    np.inf)
        ...
        print(
            "joints controlled in this demo: {}".format(
                self.ctrl_joint_names))
    ...
    def _step(self, action):
        ...
        """
        # Clip by veloctiy
        # action = self._action_clip(action)  # should we used incremental
        # command for a?
        ...
        print(
            "=================step: %d, reward : %.3f, current dist: %.3f" %
            (self.time_step_index, reward, distance))
    ...
    def _action_clip(self, action):
        ...
        safe_coef = 0.4
        pos_upper = np.array(self.joint_pos) + np.array(
            self.__joint_vel_limits_upper) * self.control_period * safe_coef
        pos_lower = np.array(self.joint_pos) + np.array(
            self.__joint_vel_limits_lower) * self.control_period * safe_coef
        ...
        if self.robot_name == 'steel':
            ...
            try:
                end_state = self.get_link_pose_srv.call(
                    'tiago_steel::arm_7_link', "base_footprint").link_state
            ...
            try:
                end_state = self.get_link_pose_srv.call(
                    'tiago_titanium::hand_mrl_link', "base_footprint").link_state
            except (rospy.ServiceException) as exc:
                print("/gazebo/get_link_state service call failed:" + str(exc))
