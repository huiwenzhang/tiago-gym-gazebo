...
class AssemblerEnv(gym.Env):
    ...
    def __init__(self, init_node=True):
        ...
        self.ee_distance_lower=[-np.inf]*3 + [-1]*4 # position [x,y,z] and quanternion (w,x,y,z)
        self.ee_distance_upper=[np.inf]*3 + [1] * 4

        self.ee_vel_lower= [-np.inf]*6
        self.ee_vel_upper= [np.inf]*6

        self.low_state = np.array(self.ee_distance_lower + self.ee_vel_lower)
        self.high_state = np.array(self.ee_distance_upper + self.ee_vel_upper)
        ...
        self.force_sensor_lower = [-np.inf]*3
        self.force_sensor_upper = [np.inf]*3
        ...
        self.home_arm_joints_position = [0.06, -0.94, 1.38, -2.01, -1.57, -0.19]
        ...
        self.safety_bound_points = np.array([[0.34087, -0.708954, 0.805475], [1.173276, -0.708954, 0.805475],
                               [1.173276, 1.046508, 0.805475], [0.34087, 1.046508, 0.805475],
                               [0.34087, -0.708954, 1.805475], [1.173276, -0.708954, 1.805475],
                               [1.173276, 1.046508, 1.805475], [0.34087, 1.046508, 1.805475]])
...
# discretize_observation() is not need for this env
    ...
    def _step(self, action):
        ...
        if contact_flag_released is False:
            if contact_flag is True:
                state, ee_absolute_translation, joint_position, joint_vel = self._get_obs()
                # stop move
        ...
        reward = max(0.0, 3.0 - distance) + norm(current_jacobian, ord=-2)/norm(current_jacobian, ord=2)
    ...
    def _get_obs(self):
        ...
        state =  end_pose_dist + end_pose_vel + force_data

        return state, ee_absolute_translation, joint_position, joint_vel
    ...
    def _reset(self):
        ...
        self._reset_robot_arm_pose_first_stage()
    ...
    def get_ee_state(self):
        ...
        try:
            end_state = self.__get_link_pose_srv.call('robot::wrist_3_link', "world").link_state
        except (rospy.ServiceException) as exc:
    ...
    def is_task_done(self, state, end_absolute_translation, joint_vel):
        ...
        task_error = norm(np.array(end_pose_dist))
        epsilon = 1e-4

        if task_error <= epsilon:
            return True
        else:
            return False
